...
- (void)presentAnimated
{
	...
	switch (self.popover.animationType) {
...
- (void)presentWithPopAnimation
{
	...
	NSRect startFrame = [self.popover popoverFrameWithSize:START_SIZE andEdge:self.frameView.edge];
	NSRect overshootFrame = [self.popover popoverFrameWithSize:NSMakeSize(endFrame.size.width * OVERSHOOT_FACTOR, endFrame.size.height * OVERSHOOT_FACTOR) andEdge:self.frameView.edge];
...
{
	if (self.popover.closesWhenEscapeKeyPressed) [self.popover performClose:nil];
}
...
- (NSWindow *)_zoomWindowWithRect:(NSRect)rect
{
  ...
  if([zoomWindow respondsToSelector: @selector(useOptimizedDrawing:)])
